...
from amt_tools.datasets import EGSet12
...
EX_NAME = '_'.join([TabCNN.model_name(),
                    'Inference/GuitarSetAndMyDataFX-TestSet_CQT_Jams_corrected',
                    CQT.features_name()])
...
def config():
    ...
    # PATH TO PRETRAINED MODEL The one here is just an example, you need to train a model first.
    
    pre_trained_model_path = '/home/pedroza/AMT-Tools/amt-tools/generated/experiments/TabCNN_GuitarSet/models/fold-5/model-2500.pt'  # Update this path
...
def tabcnn_cross_val(sample_rate, hop_length, num_frames,
                     ...
                     gpu_id, reset_data, seed, root_dir, pre_trained_model_path):
    ...
    for k in range(6):
        ...
        # train_splits = GuitarSetAndMyData.available_splits()
        # del train_splits[k]
        test_splits = TestSet.available_splits()
        test_splits = [test_splits.pop(k)]

        print(f'Loading testing partition (player {test_splits[0]})...')

        # Create a dataset corresponding to the testing partition
        gset_test = TestSet(base_dir=None,
                            splits=test_splits,
                            hop_length=hop_length,
        ...
        loaded_model = torch.load(pre_trained_model_path, map_location=torch.device('cuda', gpu_id) if gpu_id >= 0 else torch.device('cpu'))
        ...
        # Copy the state dictionary from the loaded model
        tabcnn.load_state_dict(loaded_model.state_dict())
        tabcnn.eval()
