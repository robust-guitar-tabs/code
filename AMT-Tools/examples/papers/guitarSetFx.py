...
from amt_tools.datasets import GuitarSetFx
...
def tabcnn_cross_val(sample_rate, hop_length, num_frames,
    ...
    for k in range(6):
        ...
        train_splits = GuitarSetFx.available_splits()
        del train_splits[k]
        test_splits = GuitarSet.available_splits()
        test_splits = [test_splits.pop(k)]

        print('Loading training partition...')

        # Create a dataset corresponding to the training partition
        gset_train = GuitarSetAndMyGuitarSetFx(base_dir=None,
                               splits=train_splits,
                               hop_length=hop_length,
